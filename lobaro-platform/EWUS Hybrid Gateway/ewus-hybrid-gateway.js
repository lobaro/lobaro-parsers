// This is v0.1.0
// Source: https://github.com/lobaro/lobaro-parsers/tree/master/lobaro-platform/EWUS%20Hybrid%20Gateway
...
function NB_ParseMBusTelegram(mbus, imei, source) {
    var oldLastReadout = Device.getProperty("platform.wmbus.lastReadout");
    var newLastReadout = new Date((mbus.time || mbus.timestamp) * 1000 || Date.now());
    ...
    var timestamp = newLastReadout.getTime()

    var wmbus = Parser.parseWmbus(parseBase64(mbus.telegram));
    var header = (source == "mbus" && wmbus.Header) ? wmbus.Header : wmbus

    // Filter Easy Meter telegrams with device info but no metering data
    if (wmbus.MFieldCodeString == "ESY" && wmbus.FormatSignature == 44758) {
        return null;
    }
    if (wmbus && wmbus.IdString) {
        NB_UpdateWmbusMeterStatistics(wmbus.IdString);
    }
    // Prefer Header.IdString if exists
    var id = 0;
    if (wmbus.Header && wmbus.Header.IdString) {
        id = wmbus.Header.IdString;
    } else {
        id = wmbus.IdString;
    }
    // Also prefer Header.DeviceType; it needs to be printed as a hexstring first, with 2 digits
    var devType;
    if (wmbus.Header && wmbus.Header.DeviceType) {
        devType = ( (wmbus.Header.DeviceType > 0x0f) ? "0x" : "0x0" ) + wmbus.Header.DeviceType.toString(16)
    } else {
        devType = wmbus.Device
    }

    var records = [];
    for (var i = 0; i < wmbus.Body.DataRecords.length; i++) {
        var rec = wmbus.Body.DataRecords[i];

        if (!rec.VifUnit) continue;

        records.push({
            "datapointId": "" + id + "_" + header.MFieldCodeString + "_" + (header.DeviceType || header.Device) + "_" + header.Version + "_" + i,
            "description": ((rec.VifQuantity || "") + " " + (rec.VifUnit || "")).trim(),
            "unit": rec.VifUnit || "?",
            "value": rec.ValueScaled,
            "time": timestamp
        });
    }
    ...
    return {
        "time": timestamp,
        "meterId": header.IdString,
        "gatewayId": imei,
        "isEncrypted": wmbus.Body.IsEncrypted,
        "manufacturer": header.MFieldCodeString,
        "type": header.DeviceString,
        "device:": devType,
...
function NB_ParseDataQuery(input) {
    ...
    if (input.d.telegram) {
        return NB_ParseMBusTelegram(input.d, input.i, input.q || "wmbus");
    }
    ...
    for (var i = 0; i < input.d.batch.length; i++) {
        var el = input.d.batch[i]
        res.push(NB_ParseMBusTelegram(el, input.i, input.q || "wmbus"))
    }

    // remove skipped values
    return res.filter(function (el) {return el != null})
}
